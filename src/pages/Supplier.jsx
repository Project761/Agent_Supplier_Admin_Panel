...
import { getPostData, GetWithToken, PostWithToken } from "../ApiMethods/ApiMethods";
...
const Supplier = () => {
    ...
    const GetData_Party = async () => {
        ...
        try {
            const res = await PostWithToken("Supply/GetData_Supply", val);
            if (res) {
                setItems(res);
            } else {
                setItems([]);
    ...
    const Delete_Supply = async (SupplyID) => {
        try {
            const val = {
                DeleteByUser: "1",
                IsActive: "0",
            ...
            const res = await PostWithToken("Supply/Delete_Supply", val);
            if (res) {
                toastifySuccess("Supplier successfully Deleted");
                await GetData_Party();
            }
        } catch (error) {
            console.error("Delete_Supply error:", error);
        }
    };
    ...
    const filteredItems = useMemo(() => {
        ...
        return items.filter((r) => {
            const hay = `${r.FullName} ${r.FirmName} ${r.MobileNo} ${r.Address}`.toLowerCase();
    ...
    const columns = useMemo(
        () => [
            ...
            {
                name: <span className="font-semibold">Email</span>,
                ...
                {deleteTarget && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center">
                        ...
                        <div className="relative z-10 w-full max-w-sm mx-4 rounded-2xl bg-white p-4 sm:p-5 shadow-xl">
                            <h2 className="text-lg font-semibold text-slate-800">Delete Supplier</h2>
                            <p className="mt-2 text-sm text-slate-600">
                                Are you sure you want to delete{" "}
                                <span className="font-semibold">{deleteTarget.Name || deleteTarget.FullName}</span>?
                            </p>
                            <div className="mt-4 flex justify-end gap-3">
                                ...
                                <button
                                    type="button"
                                    onClick={async () => {
                                        await Delete_Supply(deleteTarget.SupplyId);
