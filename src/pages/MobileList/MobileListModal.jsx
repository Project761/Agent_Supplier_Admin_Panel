...
const MobileListModal = ({ open, onClose, onSave, editData, onSuccess }) => {
    ...
    const Check_validate = () => {
        ...
        if (Object.keys(newErrors)?.length === 0) {
            if (editData?.MobileNoID) {
                Update_MobileNo(editData?.MobileNoID)
            } else {
                Insert_MobileNo();
            }
        }
    ...
    const Insert_MobileNo = async () => {
        try {
            const res = await PostWithToken('MobileNo/Insert_MobileNo', value)
            if (res[0]?.Message == "MobileNo created successfully") {
                onClose?.();
                onSuccess?.();
                toastifySuccess('Mobile No inserted successfully');
                setvalue({
                    CustomerName: "",
                    MobileNo: "",
                });
                setErrors({});
            } else if (res[0]?.Message == "Mobile number already exists") {
                setErrors({ MobileNo: "Mobile number already exists" });
            }
        } catch (error) {
    ...
    const Update_MobileNo = async (MobileNoID) => {
        try {
            const { CustomerName, MobileNo, Remarks } = value;
            const val = {
            ...
            const res = await PostWithToken('MobileNo/Update_MobileNo', val)
            if (res) {
                onClose?.();
                onSuccess?.();
                toastifySuccess('Mobile No Update successfully');
                setvalue({
                    CustomerName: "",
                    MobileNo: "",
                });
                    ...
                    <div className="p-3 sm:p-4 md:p-6">
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="text-lg sm:text-xl font-semibold text-slate-800">
                                {editData ? "Update Mobile No" : "Add Mobile No"}
                            </h2>
                            <button
                                type="button"
                                onClick={onClose}
                                ...
                                <div className="flex flex-col sm:col-span-2 lg:col-span-1">
                                    ...
                                    <div className="space-y-1">
                                        <input
                                            ...
                                            onChange={handleSupplierChange("CustomerName")}
                                ...
                                <div className="flex flex-col sm:col-span-2 lg:col-span-1">
                                    ...
                                    <div className="space-y-1">
                                        <input
                                            type="text"
                                            value={value.Remarks}
                                            onChange={handleSupplierChange("Remarks")}
                                            autoComplete="off-district"
