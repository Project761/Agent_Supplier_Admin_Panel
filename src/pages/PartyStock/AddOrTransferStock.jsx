...
import InItamModal from "../stoked/InItamModal";
...
const AddOrTransferStock = () => {
  ...
  const GetData_Adminuser = async () => {
    ...
    try {
      const res = await PostWithToken("PartyStock/GetData_LocationbyStock", val);
      // console.log(res, 'res')
  ...
  const columns = [
    {
      name: <span className="font-semibold">Location Name</span>,
    ...
    {
      name: <span className="font-semibold">Total Out Stock</span>,
      selector: (row) => row.TotalOutStock || "-",
      sortable: true,
    },
    {
      name: <span className="font-semibold">Remaining Quentity</span>,
...
export default AddOrTransferStock;